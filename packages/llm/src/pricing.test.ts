...
describe('estimateCost', () => {
  it('computes cost from the table', () => {
    expect(estimateCost({ inputTokens: 1_000_000, outputTokens: 0 }, 'gpt-4o', table)).toBeCloseTo(
      2.5,
    );
...
describe('cached input tokens', () => {
  ...
  it('prices the real glm-5.3 rates', () => {
    const c = estimateCost({ inputTokens: 1_000_000, outputTokens: 1_000_000 }, 'glm-5.3', {
      'glm-5.3': glm,
    });
  ...
  it('changes nothing when the cached rate equals the input rate', () => {
    const withCache = estimateCost(
      { inputTokens: 1e6, outputTokens: 0, cachedInputTokens: 5e5 },
      'g',
      { g: glm },
    );
  ...
  it('defaults the cached rate to the input rate when the model omits it', () => {
    ...
    expect(
      estimateCost({ inputTokens: 1e6, outputTokens: 0, cachedInputTokens: 1e6 }, 'x', {
        x: noCacheField,
      }),
    ).toBeCloseTo(2);
  });
  ...
  it('never lets a bogus cached count exceed the prompt total or go negative', () => {
    const over = estimateCost(
      { inputTokens: 1000, outputTokens: 0, cachedInputTokens: 99_999 },
      'm',
      { m: discounted },
    );
    expect(over).toBeCloseTo((1000 / 1e6) * 0.28);
    const neg = estimateCost({ inputTokens: 1000, outputTokens: 0, cachedInputTokens: -5 }, 'm', {
      m: discounted,
    });
...
describe('the shipped table', () => {
  it('prices the two GLM models the maintainer supplied', () => {
    const t = loadPriceTable('config/pricing.yaml');
    expect(t['glm-5.3']).toEqual({
      inputPerMTok: 2.8,
      outputPerMTok: 8.8,
      cachedInputPerMTok: 2.8,
    });
    expect(t['glm-5.3-flash']).toEqual({
      inputPerMTok: 0.6,
      outputPerMTok: 2,
      cachedInputPerMTok: 0.6,
    });
    expect(t['gemini-3.8-flash']).toEqual({
      inputPerMTok: 1.5,
      outputPerMTok: 7.5,
