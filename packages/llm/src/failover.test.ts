...
const failing = (
  name: string,
  code: 'TOOL_TIMEOUT' | 'TOOL_UNAVAILABLE' | 'INTERNAL' | 'CONFIG_INVALID',
...
describe('withFailover', () => {
  ...
  it('propagates a non-failover error rather than trying the fallback', async () => {
    const fb = vi.fn(() => Promise.resolve(ok('fallback')));
    const p = withFailover({
      primary: failing('primary', 'CONFIG_INVALID'),
      fallback: fake('fallback', fb),
    });
    await expect(
      p.complete({ model: 'm', messages: [] }, new AbortController().signal),
    ).rejects.toMatchObject({
  ...
  it('does not fail over after the caller has aborted — failover buys no extra time', async () => {
    ...
    const fb = vi.fn(() => Promise.resolve(ok('fallback')));
    const p = withFailover({
      primary: failing('primary', 'TOOL_TIMEOUT'),
      fallback: fake('fallback', fb),
    });
  ...
  it('reports capabilities as the intersection', async () => {
    const p = withFailover({
      ...
      fallback: fake('fallback', () => Promise.resolve(ok('fallback')), {
        tools: false,
        streaming: true,
      }),
    });
